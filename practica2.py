...
PlotContourf_SA(data=prec_regresion,
                data_var=prec_regresion.sel(coef='nino34_1').prec,
                scale=np.arange(0,180, 20),
                cmap='Blues', title='Coef. Niño3.4 - Prec. SON',
                mask_andes=True, mask_land=False)

PlotContourf_SA(data=prec_regresion_anomalias,
                data_var=prec_regresion_anomalias.sel(coef='nino34_1'),
...
PlotContourf_SA(data=prec,
                data_var=prec.sel(time='2015-10-01').squeeze().prec,
                scale=np.arange(0,200, 20),
                cmap='Blues', title='Prec. observada - SON-2015',
                mask_andes=True, mask_land=False)

PlotContourf_SA(data=prec_reconstruccion,
                data_var=prec_reconstruccion.sel(time='2015-10-01').squeeze().prec,
                scale=np.arange(0,200, 20),
                cmap='Blues', title='Reconstrucción MLR  Prec. - SON-2015',
                mask_andes=True, mask_land=False)

# ---------------------------------------------------------------------------- #
# Ejemplo con lags que requieren predictores en AÑO DIFERENTE al predictando
# DMI en octubre, N34 en noviembre, SAM en enero y Prec. en FMA
...
prec_regre, prec_regre_anoms, reconstruct = Compute_MLR(
    predictando=prec,
    mes_predictando=3, # Mar
    predictores = [n34, dmi, sam],
...
PlotContourf_SA(data=prec_reconstruccion_tt,
                data_var=prec_reconstruccion_tt.sel(time='2015-10-01').squeeze().prec,
                scale=np.arange(0, 200, 20),
                cmap='Blues',
...
# De esta forma recosntruye el periodo completo
...
PlotContourf_SA(acc_result,
                acc_result.prec,
                scale=np.arange(-1, 1.2, 0.2),
                cmap='BrBG', title='ACC - Prec. observada vs reconstruccion MLR',
                mask_andes=True, mask_land=False)
...
PlotContourf_SA(prec_reconstruccion_cv,
                ...
                scale=np.arange(0, 200, 20),
                cmap='Blues', title='Reconstrucción MLR CV Prec. - SON-2015',
                mask_andes=True, mask_land=False)
...
P, Q, A, B, S = Compute_CCA(
    ...
    X_mes=9, Y_mes=11, # meses que se usaran en cada una
    X_trim=True, Y_trim=True, # trimestral, si se selecciono X_mes = 9 --> 8-9-10
